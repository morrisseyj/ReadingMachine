...
The orphan count remains high across both passes. A large proportion of insights are not identified as being represented in theme summaries, even after reinsertion. Given the content of the output—which qualitatively presents as a high‑quality, coherent literature review—this strongly suggests that the orphan detection prompt is overly strict, rather than that theme summarization or reinsertion is catastrophically failing.
This is a preferable failure mode. Over‑detection of orphans biases the system toward false positives and reduces the risk of silent omission, whereas under‑detection would mask genuine coverage loss.

A second, non‑exclusive explanation is that high orphan counts may function as a diagnostic signal of synthesis stress. Under conditions of heavy synthesis pressure, theme summaries tend toward higher levels of abstraction. As abstraction increases, individual insight claims are less likely to appear in surface‑proximal or near‑verbatim form, causing the orphan check to return negative results even when the substance has been integrated.

In this interpretation, elevated orphan counts may indicate that insight density within a theme is approaching a scaling limit for reliable explicit integration, rather than representing a simple failure of theme summarization or reinsertion. The behavior observed here is likely the interaction of both effects: overly strict orphan detection rules interacting with abstraction pressure at the theme level.

Importantly, this interpretation is grounded in inspection of the output rather than a formal system evaluation. Systematic evaluation is intentionally deferred to the open review process; if external reviewers judge the output to be substantively strong, the appropriate question becomes how the method produced that result under these conditions, rather than whether the method has failed.
