...
p <- ggplot() +
  ...
  coord_cartesian(
    xlim = c(-4.5, 4.1),
    ylim = c(min(steps$y) - 0.3, max(steps$y) + 0.3),
    expand = FALSE,
    clip = "off"
  ) +
...
ggsave("C:\\Users\\jmorrissey\\Documents\\python_projects\\ReadingMachine\\documentation\\whitepaper_specifications\\readingmachine_pipeline.pdf", 
       p, width = 7, height = 10, device = cairo_pdf)
